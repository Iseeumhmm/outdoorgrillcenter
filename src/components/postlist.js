      ...
      <div className={cx('group cursor-pointer', minimal && 'grid gap-10 md:grid-cols-2')}>
        ...
        >
          <Link
            ...
            href={`/reviews/${pathPrefix ? `${pathPrefix}/` : ''}${post.slug}`}
          >
            {imageProps ? (
              <Image
                src={imageProps.src}
        ...
        <div className={cx(minimal && 'flex items-center')}>
          <div>
            ...
            >
              <Link href={`/reviews/${pathPrefix ? `${pathPrefix}/` : ''}${post.slug}`}>
                <span className="bg-gradient-to-r from-green-200 to-green-100 bg-[length:0px_10px] bg-left-bottom bg-no-repeat transition-[background-size] duration-500 hover:bg-[length:100%_3px] group-hover:bg-[length:100%_10px] dark:from-bbq-orange dark:to-bbq-fire-dark">
                  {post.title}
                </span>
              </Link>
            </h2>
